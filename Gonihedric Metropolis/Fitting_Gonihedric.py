...
energy_timeseries=np.asarray(np.loadtxt("metropolis_data/" + "3d_energy_timeseries_" + str(5.0) + "_L" + str(20) + ".txt"))
tau=np.loadtxt(
...
Y=np.asarray(np.loadtxt("metropolis_data/" + "3d_energy_timeseries_" + str(T) + "_L" + str(L) + ".txt")[80::16])
X=np.asarray([np.loadtxt("metropolis_data/" + "3d_C_1_timeseries_" + str(T) + "_L" + str(L) + ".txt")[80::16],np.loadtxt("metropolis_data/" + "3d_C_2_timeseries_" + str(T) + "_L" + str(L) + ".txt")[80::16]])
...
plt.ylabel('Energy/N')
plt.legend()
plt.savefig('Graph of fitting with all parameters_3d_plaq_periodic.png')
plt.close()
